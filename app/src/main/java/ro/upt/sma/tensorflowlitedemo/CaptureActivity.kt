    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cmv_camera_preview.setOnClickListener { _ ->
            GlobalScope.launch((Dispatchers.Default)) {
                ...
                val recognition = classifier.recognize(newBitmap);
                withContext(Dispatchers.Main) {
                    // Show the recognitions using the common Toast widget. Make use of joinToString method to concat multiple items.
                    val showRecognition = recognition.joinToString {
                            recognition ->
                        recognition.label + " " + recognition.confidence;
                    }
                    Toast.makeText(applicationContext, showRecognition, Toast.LENGTH_LONG).show();
                }
